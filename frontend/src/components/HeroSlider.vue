...
<script setup>
  ...
  const scrollToLandingBulk = () => {
    const landingElement = document.getElementById("landing-page-bulk");
    if (landingElement) {
      landingElement.scrollIntoView({ behavior: "smooth" });
    } else {
      console.error("Site contents of landing page not found.");
    }
  };
</script>
...
<template>
  <section class="relative h-screen w-full text-white z-10">
    ...
    <div
      ...
      :style="{ backgroundImage: `url(${nismedHero})` }"
    ></div>
    ...
    <button
      ...
      class="cursor-pointer absolute bottom-20 left-1/2 transform -translate-x-1/2 animate-bounce z-30 focus:outline-none"
